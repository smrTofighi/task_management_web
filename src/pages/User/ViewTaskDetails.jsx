...
export const ViewTaskDetails = () => {
  ...
  const getTaskDetialsById = async () => {
    try {
      ...
      console.error("Error to get task details :", error);
    }
  };
  // handle todo check
    ...
    if(todoChecklist && todoChecklist[index]){
        ...
        try {
            ...
            if(response.status ===200){
                ...
                todoChecklist[index].completed = ! todoChecklist[index].complete
            }
        } catch (error) {
            todoChecklist[index].completed = ! todoChecklist[index].complete
            console.log('Error to update todoList :',error);
  ...
  const handleLinkClick = (link) => {
    if(!/^https?:\/\//i.test(link)){
        link = "https://" + link; // Defualt to HTTPS
    }
